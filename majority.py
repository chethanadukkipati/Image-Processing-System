...
def majority(image, str_ele, str_ele_shape):
    """
    THE ALGORITHM EXCPECTS BACKGROUND TO BE DARKER THAN THE FORGROUND / OBJECT \n
    perform dilation on given image with given structuring element: \n
    image: input image \n
    ...
    dilated_image = numpy.copy(image)
    dilated_image_channels = numpy.copy(image_channels)

    # pad channels with NaN
    for i in range(len(image_channels)):
        image_channels[i] = numpy.pad(image_channels[i].astype(float),
                                      [(top_padding, bottom_padding), (left_padding, right_padding)], mode='constant',
    ...
    for c in range(len(image_channels)):
        ...
        arr = [
            image_channels[c][top_padding: top_padding + image.shape[0], left_padding: left_padding + image.shape[1]]]

        # cross str_ele
        if (str_ele_shape == 'cross'):

            for x in range(len(str_ele) // 2):
                arr.append(image_channels[c][top_padding + x + 1: top_padding + x + 1 + image.shape[0],
                           left_padding: -right_padding])
                arr.append(image_channels[c][top_padding + x - 1: top_padding + x - 1 + image.shape[0],
                           left_padding: -right_padding])
                arr.append(image_channels[c][top_padding: -bottom_padding,
                           left_padding + x + 1: left_padding + x + 1 + image.shape[1]])
                arr.append(image_channels[c][top_padding: -bottom_padding,
                           left_padding + x - 1: left_padding + x - 1 + image.shape[1]])

            dilated_image_channels[c] = numpy.full_like(dilated_image_channels[c], fill_value=0, dtype=numpy.uint8)
            dilated_image_channels[c][:, :] = numpy.nanmedian(numpy.stack(arr, axis=0), axis=0)
        ...
        # otherwise
            ...
            for y in range(len(dilated_image_channels[c])):
                for x in range(len(dilated_image_channels[c][0])):
                    dilated_image_channels[c][y][x] = numpy.nanmedian(
                        str_ele * image_channels[c][y:y + len(str_ele), x:x + (len(str_ele[0]))])

    dilated_image = cv2.merge(dilated_image_channels)
