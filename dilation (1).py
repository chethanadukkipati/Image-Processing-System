...
def dilation (image, str_ele, str_ele_shape = "other"):
    """
    ...
    for c in range(len(image_channels)):
        ...
        if (str_ele_shape == 'cross'):
            ...
            dilated_image_channels[c][:, :] = numpy.nanmax(numpy.stack(arr,axis = 0), axis=0)

        #square
        elif(str_ele_shape == 'square'):
            if (str_ele.shape[0] > 1):
                for y in range(len(str_ele)):
                    for x in range(len(str_ele[0])):
        ...
        #otherwise
            ...
            for y in range(len(dilated_image_channels[c])):
                for x in range(len(dilated_image_channels[c][0])):
                    dilated_image_channels[c][y][x] = numpy.nanmax(str_ele*image_channels[c][y:y+len(str_ele),x:x+(len(str_ele[0]))])
