...
def erosion (image, str_ele, str_ele_shape = "other"):
    """
    perform erosion on given image with given structuring element using min: \n
    please specify str_ele_shape for significant performance boost.

    :param image: input image \n
    :param str_ele: 2D numpy array taking values non zero positive and NaN \n
    :param str_ele_shape: 'square' iff str ele square, positive odd dim, filled with 1 and NaN.\n'cross' iff str ele cross, positive odd dim, filled with 1 and NaN.\nleave empty otherwise. may be very slow.
    
    other supported str_eles: any shape/size with values(weights) between 0-1, & NaN.
    output: eroded image of the same shape as the input image\n
    """
